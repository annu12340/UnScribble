...
export function renderInsightSections(med, els, renderTagList) {
  ...
  const hasRegulatoryInfo = Object.values(regulatoryLists).some(
    (list) => list.length > 0,
  );
  ...
  const activeIngredient =
    ...
    ingredientData.active_ingredient ||
    med.medication_name ||
    "—";
  ...
  const duplicateWarnings =
    med.duplicate_ingredient_warnings ||
    ingredientData.duplicate_ingredient_warnings ||
    [];

  if (els.medActiveIngredient) {
    els.medActiveIngredient.textContent = activeIngredient;
  }
  ...
  const interactingMeds = Array.isArray(
  ...
  const foodSupplements = Array.isArray(interactions.food_supplements_to_avoid)
    ...
    : [];
  const contraindications = Array.isArray(
  ...
  if (els.medAgePrecautions) {
    els.medAgePrecautions.textContent =
      safetyFlags.age_based_precautions || "—";
  }
  if (els.medAllergyRiskSummary) {
    els.medAllergyRiskSummary.textContent =
      safetyFlags.allergy_risk_summary || "—";
  }
  ...
  if (els.medStorageInstructions) {
    els.medStorageInstructions.textContent =
      administration.storage_instructions || "—";
  }
  if (els.medMissedDoseGuidance) {
    els.medMissedDoseGuidance.textContent =
      administration.missed_dose_guidance || "—";
  }

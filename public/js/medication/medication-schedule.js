...
function debugWarn(...args) {
  if (DEBUG_MEDICATION_SCHEDULE) console.warn(...args);
}

function debugError(...args) {
  if (DEBUG_MEDICATION_SCHEDULE) console.error(...args);
}
...
export function initGoogleCalendar() {
  debugLog("[Google Calendar] Initializing Google Calendar API...");
  ...
  gapiInitPromise = new Promise((resolve, reject) => {
    ...
    if (window.gapi && window.gapi.client && window.google?.accounts) {
      debugLog("[Google Calendar] Google API already fully loaded");
    ...
    const checkBothLoaded = () => {
      if (gapiLoaded && gisLoaded) {
        debugLog("[Google Calendar] Both Google API and GIS loaded");
        gapiInitialized = true;
        resolve();
      }
    };
    ...
    if (window.gapi && window.gapi.load) {
      debugLog("[Google Calendar] Google API exists, loading client...");
      window.gapi.load("client", {
        callback: () => {
          debugLog("[Google Calendar] Google API client loaded");
          gapiLoaded = true;
          checkBothLoaded();
        },
        onerror: (error) => {
      ...
      gapiScript.onload = () => {
        debugLog("[Google Calendar] Google API script loaded successfully");
        if (!window.gapi) {
          debugError("[Google Calendar] gapi not available after script load");
          gapiInitPromise = null;
    ...
    if (window.google?.accounts) {
      debugLog("[Google Calendar] Google Identity Services already loaded");
      gisLoaded = true;
      checkBothLoaded();
    } else {
      debugLog("[Google Calendar] Loading Google Identity Services...");
...
export async function authenticateGoogleCalendar(clientId) {
  ...
  if (!gapiClientInitialized) {
    debugLog("[Google Calendar] Loading Calendar API...");
    try {
      // Load Calendar API (no API key needed for OAuth-only APIs)
      await window.gapi.client.load("calendar", "v3");
      debugLog("[Google Calendar] Calendar API loaded");
      ...
      if (error.details) {
        throw new Error(`Google API Error: ${error.details}`, { cause: error });
      } else if (error.error) {
        throw new Error(`Google API Error: ${error.error}`, { cause: error });
      } else {
        throw new Error(
  ...
  if (!tokenClient) {
    debugLog("[Google Calendar] Initializing OAuth2 token client...");
    tokenClient = google.accounts.oauth2.initTokenClient({
      client_id: clientId,
      scope: "https://www.googleapis.com/auth/calendar.events",
      callback: (response) => {
        if (response.error !== undefined) {
          debugError("[Google Calendar] OAuth error:", response.error);
          throw response;
        }
  ...
  if (accessToken) {
    debugLog("[Google Calendar] Already have access token");
    return true;
  }

  // Request access token
  debugLog("[Google Calendar] Requesting access token...");
  return new Promise((resolve, reject) => {
    ...
    tokenClient.callback = (response) => {
      ...
      accessToken = response.access_token;
      debugLog("[Google Calendar] Access token received successfully");
      resolve(true);
    };
    ...
    try {
      ...
      debugError("[Google Calendar] Error requesting access token:", error);
      reject(error);
    }
  });
}
...
export function isGoogleSignedIn() {
  ...
  debugLog("[Google Calendar] Sign-in check:", signedIn);
  return signedIn;
}

export async function signOutGoogle() {
  debugLog("[Google Calendar] Signing out...");
  if (accessToken && window.google?.accounts?.oauth2) {
    try {
      google.accounts.oauth2.revoke(accessToken, () => {
        debugLog("[Google Calendar] Access token revoked");
      });
      accessToken = null;
      debugLog("[Google Calendar] Signed out successfully");
    } catch (error) {
      debugError("[Google Calendar] Error during sign out:", error);
      accessToken = null;
    }
  } else {
    debugWarn("[Google Calendar] No active session to sign out from");
    accessToken = null;
  }
}

export function getGoogleUserInfo() {
  debugLog("[Google Calendar] Getting user info...");
  if (!isGoogleSignedIn()) {
    debugWarn("[Google Calendar] User not signed in");
    return null;
  }
  ...
  debugLog("[Google Calendar] User info not available with token-based auth");
  return null;
}
...
export async function addMedicationToGoogleCalendar(
  ...
  debugLog("[Google Calendar] Start date:", startDate);
  debugLog("[Google Calendar] Schedule times:", schedule.schedule.times.length);
  ...
  try {
    debugLog("[Google Calendar] Authenticating...");
    const isAuthenticated = await authenticateGoogleCalendar(clientId);
    if (!isAuthenticated) {
      debugError("[Google Calendar] Authentication failed");
      throw new Error("Failed to authenticate with Google Calendar");
    }
    debugLog("[Google Calendar] Authentication successful");

    // Set the access token for API requests
    if (accessToken) {
      window.gapi.client.setToken({ access_token: accessToken });
      debugLog("[Google Calendar] Access token set for API requests");
    }

    const duration = parseDuration(schedule.duration);
    debugLog("[Google Calendar] Parsed duration:", duration.days, "days");
    ...
    for (let i = 0; i < schedule.schedule.times.length; i++) {
      ...
      if (!timeSlot.time) {
        debugLog('[Google Calendar] Skipping "as needed" medication');
        continue;
      }
      ...
      try {
        ...
        debugError("[Google Calendar] Failed to insert event:", insertError);
        debugError("[Google Calendar] Error details:", {
    ...
    debugLog("[Google Calendar] ========================================");
    debugError("[Google Calendar] Error adding to Google Calendar:", error);
    debugError("[Google Calendar] Error type:", error.constructor.name);
    debugError("[Google Calendar] Error message:", error.message);
    debugError("[Google Calendar] Error details:", error.details);
    debugError("[Google Calendar] Error result:", error.result);
    debugError("[Google Calendar] Error status:", error.status);
    if (error.result) {
      debugError("[Google Calendar] API error details:", error.result);
    }
    debugError("[Google Calendar] ========================================");
    throw error;
  }
}

export function createCalendarEvent(schedule, timeSlot, startDate, duration) {
  debugLog("[Google Calendar] Creating calendar event...");
  debugLog("[Google Calendar] Time slot:", timeSlot.time, "-", timeSlot.label);
  ...
  debugLog("[Google Calendar] Timezone:", timeZone);
  debugLog("[Google Calendar] Event start:", eventStart.toISOString());
  debugLog("[Google Calendar] Event end:", eventEnd.toISOString());
  ...
  if (duration.days > 1) {
    ...
    debugLog("[Google Calendar] Added recurrence:", rrule);
    debugLog("[Google Calendar] Recurrence until:", until.toISOString());
  } else {
    debugLog("[Google Calendar] Single event (no recurrence)");
  }
...
export function parseDuration(durationStr) {
  debugLog("[Google Calendar] Parsing duration:", durationStr);

  if (!durationStr) {
    debugLog("[Google Calendar] No duration specified, defaulting to 7 days");
    return { days: 7 };
  }
  ...
  if (str.includes("day")) {
    ...
    debugLog("[Google Calendar] Parsed days:", days);
    return { days };
  }
  ...
  if (str.includes("week")) {
    ...
    debugLog("[Google Calendar] Parsed weeks to days:", days);
    return { days };
  }
  ...
  if (str.includes("month")) {
    ...
    debugLog("[Google Calendar] Parsed months to days:", days);
    return { days };
  }

  debugLog("[Google Calendar] Could not parse duration, defaulting to 7 days");
  return { days: 7 };
}
...
export function exportScheduleAsICS(schedules, startDate) {
  debugLog("[ICS Export] ========================================");
  debugLog("[ICS Export] Exporting schedule as ICS");
  debugLog("[ICS Export] Number of medications:", schedules.length);
  debugLog("[ICS Export] Start date:", startDate);
  ...
  debugLog("[ICS Export] Export complete");
  debugLog("[ICS Export] ========================================");
}

export function generateICS(events) {
  debugLog("[ICS Export] Generating ICS format for", events.length, "events");
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    debugLog(`[ICS Export] Adding event ${i + 1}/${events.length} to ICS`);
    ...
    if (event.rrule) {
      ...
      debugLog(`[ICS Export]   Added recurrence rule: ${event.rrule}`);
    }
  ...
  debugLog("[ICS Export] ICS generation complete, total lines:", lines.length);
  return lines.join("\r\n");
}
...
function downloadICS(content, filename) {
  debugLog("[ICS Export] Creating blob and download link");
  debugLog("[ICS Export] Filename:", filename);

  const blob = new Blob([content], { type: "text/calendar;charset=utf-8" });
  debugLog("[ICS Export] Blob created, size:", blob.size, "bytes");

  const url = URL.createObjectURL(blob);
  debugLog("[ICS Export] Object URL created:", url);

  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = filename;
  anchor.click();
  debugLog("[ICS Export] Download triggered");

  URL.revokeObjectURL(url);
  debugLog("[ICS Export] Object URL revoked");
}
